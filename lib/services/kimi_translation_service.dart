...
class KimiApiException implements Exception {
  ...
  /// Thrown when the user cancels an in-progress translation.
  ...
  class KimiTranslationService {
    ...
    ///
    /// Long chapters are chunked by paragraphs so the model stays within
    /// context limits and paragraph order is preserved for the bilingual reader.
    ///
    /// [onProgress] is called after each successful chunk with
    /// (current 1-based index, total chunks, partial joined translation).
    /// [shouldCancel] is polled before each chunk; if it returns true the
    /// remaining API calls stop and [TranslationCancelledException] is thrown
    /// (already-finished chunks are available via [onProgress]).
    Future<String> translateText(
      String rawText, {
      int maxRetries = 2,
      void Function(int current, int total, String partialTranslation)? onProgress,
      ...
      for (var i = 0; i < chunks.length; i++) {
        ...
        final partial = translatedChunks.join('\n\n');
        onProgress?.call(i + 1, chunks.length, partial);
      }
      ...
      for (var attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          ...
          if (!retryable || attempt == maxRetries) {
            final where = (chunkIndex != null && chunkTotal != null)
                ? ' (chunk $chunkIndex/$chunkTotal)'
                : '';
            throw KimiApiException('${e.message}$where', statusCode: e.statusCode);
          }
          ...
          if (attempt == maxRetries) {
            final where = (chunkIndex != null && chunkTotal != null)
                ? ' (chunk $chunkIndex/$chunkTotal)'
                : '';
            throw KimiApiException('${e.message ?? 'Network error'}$where');
          }
          await Future<void>.delayed(Duration(seconds: 2 * (attempt + 1)));
        }
      }
    ...
    Future<String> _translateChunk(String chunk, String apiKey) async {
      try {
        final response = await _dio.post(
          ...
          data: {
            'model': _model,
            // Current international Kimi models require temperature == 1.
            'temperature': 1,
            'messages': [
